...
async def agent_stream_generator(
    ...
    # Embedding failure must not break the user's request: log it, skip the
    # cache for this request only, and fall through to the LLM path.
    try:
        cached_response = query_semantic_cache(prompt, command)
    except Exception as e:
        logger.warning("Semantic cache lookup skipped (embedding failure): %s", e)
    ...
    while True:
        ...
        if chunk is None:
            ...
            if accumulated_final_text.strip():
                try:
                    ...
                    logger.warning("Semantic cache write skipped (embedding failure): %s", e)
        ...
        try:
            ...
            logger.warning("Failed to capture response text for semantic cache: %s", e)
